...
1. 반복 측정(N≥5)으로 평균·표준편차 산출 및 박스플롯/에러바 시각화.
2. **GPU 최적화 적용**: TensorRT/FP16·INT8, 적절한 배치 사이즈 탐색, I/O 파이프라인 병렬화. (G5 권장)
3. **R5 활용 시**: 대규모 인메모리 캐시·DB·분석에 집중, EBS 고성능이 필요하면 **R5b** 고려.
4. **M5 활용 시**: 범용 워크로드 또는 GPU 미적용 환경의 LLM 추론에 비용 대비 성능 우수.
